...
class class_MyClass:
	...
	def myMethod(self):
		logging.info("run method ---->   @class:MyClass.@method:MyClass_myMethod")
		try:
			return self._myMethod()
		except Exception as e:
			message = e.message if hasattr(e, "message") else str(e)
			class_ = "@class:MyClass"
			method = "@method:MyClass_myMethod"
			raise MethodRuntimeException("An Exception in @class:MyClass during method @method:MyClass_myMethod\n@class:MyClass\n@method:MyClass_myMethod\n" + message)
...
class event_Initialize:
	...
	def _Action(self):
		objects.MyObject.myProperty = "Total cruel"
		pass
