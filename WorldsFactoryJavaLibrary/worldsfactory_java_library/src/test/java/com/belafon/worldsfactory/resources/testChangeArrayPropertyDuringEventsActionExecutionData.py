...
class Objects:
	...
	def __new__(cls):
		if cls._instance is None:
			...
			cls.MyObject.myArray.set_objects_name("@object:MyObject")
...
class event_myEvent:
	...
	def _Action(self):
		logging.info('Setting myArray[0][0] to "Array myArray set from myEvent Action method"')
		objects.MyObject.myArray[0][0] = "Array myArray set from myEvent Action method"
		pass
		pass
...
def set_array_property(propertyName, value, type, index, objectName):
	...
	logging.info(f"Setting {objectName}.{propertyName}{index} to {value}")
